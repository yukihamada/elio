...
class SherpaOnnxOfflineTtsWrapper {
  ...
  /// Returns true if the TTS was initialized successfully
  var isValid: Bool {
    return tts != nil
  }
  ...
  init(
    ...
    if tts == nil {
      print("[SherpaOnnx TTS] ERROR: Failed to create offline TTS - SherpaOnnxCreateOfflineTts returned nil")
    }
  }
  ...
  func generate(text: String, sid: Int = 0, speed: Float = 1.0) -> SherpaOnnxGeneratedAudioWrapper {
    guard tts != nil else {
      print("[SherpaOnnx TTS] ERROR: Cannot generate audio - TTS not initialized")
